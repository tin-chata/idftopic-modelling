...
class Embs(nn.Module):
    ...
    def get_embs(self, inputs, auxiliary_embs=None):
        ...
        if auxiliary_embs is not None:
            assert embs.shape[:-1] == auxiliary_embs.shape[:-1]
            embs = torch.cat([embs, auxiliary_embs], -1)
        return embs
...
class Autoencoder(nn.Module):
    ...
    def __init__(self, HPs, idf_embs=None, kmean_file=None):
        ...
        self.idf_layer = Embs([emb_size, 1, idf_embs, -1, emb_zero_padding, False])
        self.norm_idf = nn.Softmax(-2)
...
if __name__ == "__main__":
    ...
    wd_embs = topic_encoder.emb_layer(inp_tensor)
    idf_embs = topic_encoder.idf_layer(inp_tensor)
